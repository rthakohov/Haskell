...
import           Construction (Name, Term (..), bound, free, fresh, alpha, beta, eta, reduce, substitute, findBnf, countApp)
import           Test.Hspec
import Data.Set
...
main = hspec $ do
    ...
    describe "Count test" testCountApp
    describe "Bnf test" testFindBnf
...
testEq :: SpecWith ()
testEq = do
    it "#1" $ varX == varX `shouldBe` True
    it "#2" $ (App (App lamKK varX) appXX1) == (App (App lamKK varX) appXX1) `shouldBe` True
    it "#3" $ (Lam "x" (App varX1 varX1)) == (Lam "x" (App varX1 varX1)) `shouldBe` True
    it "#4" $ varX == varX1 `shouldBe` True
    it "#5" $ varX /= varX1 `shouldBe` False
    it "#6" $ Lam "x" varX == Lam "y" (Var "y") `shouldBe` True
    it "#7" $ (App lamK lamId) == (App (App lamId lamId) lamKK) `shouldBe` True

testCountApp :: SpecWith ()
testCountApp = do
    it "#1" $ (countApp (App varX varX)) `shouldBe` 1
    it "#2" $ (countApp (beta (App (Lam "x" (App varX varX)) (Lam "x" (App varX varX))))) `shouldBe` 3

testFindBnf :: SpecWith ()
testFindBnf = do
    it "#1" $ (findBnf varX) `shouldBe` True
    it "#2" $ (findBnf (App (Lam "x" varX) (Lam "x" varX)))  `shouldBe` True -- (\x.x)(\x.x)
    it "#4" $ (findBnf (App (Lam "x" (App varX varX)) (Lam "x" (App varX varX))))  `shouldBe` False -- (\x.xx)(\x.xx)
    it "#3" $ (findBnf (App (Lam "x" (App varX (App varX varX))) (Lam "x" (App varX (App varX varX)))))  `shouldBe` False -- (\x.xxx)(\x.xxx)
